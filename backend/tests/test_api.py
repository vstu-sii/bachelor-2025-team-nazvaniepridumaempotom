...
class TestDishEndpoints:
    def test_create_dish_success(self, client, auth_headers):
        ...
        files = {"photo": ("test.jpg", test_file_content, "image/jpeg")}
        ...
        response = client.post(
            "/api/dishes/", headers=auth_headers, data=data, files=files
        )
    ...
    def test_create_dish_short_recipe(self, client, auth_headers):
        ...
        data = {"dish_type": "dinner", "user_recipe_text": "Short"}  # Менее 50 символов
    ...
    def test_get_dishes_with_status_filter(self, client, auth_headers, test_dish):
        ...
        response = client.get(
            "/api/dishes/", headers=auth_headers, params={"status": "draft"}
        )
    ...
    def test_analyze_dish_success(self, client, auth_headers, test_dish):
        ...
        with patch(
            "backend.services.ai_service.AIService.analyze_dish"
        ) as mock_analyze:
            mock_analysis = Mock()
            ...
            response = client.post(
                f"/api/dishes/{test_dish.id}/analyze", headers=auth_headers
            )
    ...
    def test_get_dish_analysis(self, client, auth_headers, test_dish):
        ...
        # Получаем анализ
        response = client.get(
            f"/api/dishes/{test_dish.id}/analysis", headers=auth_headers
        )
    ...
    def test_update_dish(self, client, auth_headers, test_dish):
        ...
        update_data = {"dish_type": "lunch", "user_recipe_text": "Updated recipe " * 10}

        response = client.put(
            f"/api/dishes/{test_dish.id}", headers=auth_headers, json=update_data
        )
    ...
    def test_update_nonexistent_dish(self, client, auth_headers):
        ...
        update_data = {"dish_type": "lunch"}

        response = client.put(
            "/api/dishes/99999", headers=auth_headers, json=update_data
        )
    ...
    def test_delete_dish(self, client, auth_headers, test_dish):
        """Тест удаления блюда"""
        response = client.delete(f"/api/dishes/{test_dish.id}", headers=auth_headers)

        assert response.status_code == status.HTTP_204_NO_CONTENT

        # Проверяем что блюдо действительно удалено
        get_response = client.get(f"/api/dishes/{test_dish.id}", headers=auth_headers)
        assert get_response.status_code == status.HTTP_404_NOT_FOUND

    def test_delete_nonexistent_dish(self, client, auth_headers):
        """Тест удаления несуществующего блюда"""
        response = client.delete("/api/dishes/99999", headers=auth_headers)
...
class TestStatisticsEndpoints:
    ...
    def test_get_dish_type_statistics(self, client, auth_headers):
        ...
        response = client.get("/api/statistics/dish-types/dinner", headers=auth_headers)
    ...
    def test_get_progress_timeline(self, client, auth_headers):
        ...
        response = client.get(
            "/api/statistics/progress", headers=auth_headers, params={"days": 30}
        )
...
class TestErrorHandling:
    ...
    def test_file_upload_error(self, client, auth_headers):
        ...
        files = {"photo": ("huge.jpg", huge_file_content, "image/jpeg")}

        data = {"dish_type": "dinner", "user_recipe_text": "Test recipe " * 10}
