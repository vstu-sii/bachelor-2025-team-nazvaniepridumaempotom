...
class TestDishCRUD:
    ...
    def test_get_dishes(self, db_session, test_user, test_dish):
        ...
        dish_data = DishCreate(
            dish_type=DishType.BREAKFAST, user_recipe_text="Another recipe " * 10
        )
    ...
    def test_get_dishes_with_limit(self, db_session, test_user):
        ...
        for i in range(5):
            dish_data = DishCreate(
                dish_type=DishType.DINNER, user_recipe_text=f"Recipe {i} " * 10
            )
            dish.create_dish(db_session, dish_data, test_user.id)
    ...
    def test_update_dish(self, db_session, test_dish):
        ...
        update_data = DishUpdate(
            dish_type=DishType.LUNCH, user_recipe_text="Updated recipe text"
        )
    ...
    def test_get_user_dishes_by_status(self, db_session, test_user):
        ...
        dish_data1 = DishCreate(dish_type=DishType.DINNER, user_recipe_text="Recipe 1")
        dish1 = dish.create_dish(db_session, dish_data1, test_user.id)

        dish_data2 = DishCreate(
            dish_type=DishType.BREAKFAST, user_recipe_text="Recipe 2"
        )
        ...
        draft_dishes = dish.get_user_dishes_by_status(
            db_session, test_user.id, DishStatus.DRAFT
        )
        ready_dishes = dish.get_user_dishes_by_status(
            db_session, test_user.id, DishStatus.READY
        )
