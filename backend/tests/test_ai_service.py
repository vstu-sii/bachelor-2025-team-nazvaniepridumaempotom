...
class TestAIServiceIntegration:
    ...
    def test_analyze_dish_with_long_recipe(self):
        ...
        long_recipe = (
            "Ингредиенты: "
            + ", ".join([f"ингредиент_{i}" for i in range(20)])
            + ". "
            + "Шаги приготовления: "
            + " ".join([f"Шаг {i}: действие" for i in range(10)])
        )
        ...
        assert result.recipe_feedback
        assert (
            "Рецепт" in result.recipe_feedback
            or "ингредиент" in result.recipe_feedback.lower()
        )
