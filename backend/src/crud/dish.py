...
from src.schemas import dish
from dish import DishCreate, DishUpdate
...
def get_dishes(
    ...
    return (
        db.query(Dish).filter(Dish.user_id == user_id).offset(skip).limit(limit).all()
    )


def create_dish(db: Session, dish: DishCreate, user_id: int) -> Dish:
    db_dish = Dish(**dish.dict(), user_id=user_id, status=DishStatus.DRAFT)
    db.add(db_dish)
    db.commit()
    db.refresh(db_dish)
    return db_dish
...
def get_user_dishes_by_status(
    db: Session, user_id: int, status: DishStatus
) -> List[Dish]:
    return db.query(Dish).filter(Dish.user_id == user_id, Dish.status == status).all()
